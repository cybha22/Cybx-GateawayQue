...
func (h *Handler) recordSuccessFor(account *config.Account, model string, inputTokens, outputTokens int, credits float64, latency time.Duration, endpoint string) {
	h.recordSuccess(inputTokens, outputTokens, credits)
	if account != nil {
...
func (h *Handler) recordFailureFor(account *config.Account, model, endpoint string, latency time.Duration, status int) {
	h.recordFailure()
	if account != nil {
		statusStr := "fail"
		if status > 0 {
